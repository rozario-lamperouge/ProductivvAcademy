...
from .forms import LoginForm, RegisterForm, ForgetForm, ModifyPwdForm, ContactForm
        ...
        if login_form.is_valid():
            ...
            if user is not None:
                if user.is_active:
                    ...
                    return render(request, 'logintest.html', {'msg': 'You have not activated your account'})
            else:
                return render(request, 'logintest.html', {'msg': 'wrong user name or password'})
        else:
            return render(request, 'logintest.html', {'login_form': login_form})
    ...
    def post(self, request):
        ...
        if register_form.is_valid():
            ...
            if UserProfile.objects.filter(email=username):
                return render(request, 'registertest.html', {'msg': 'This mailbox is already registered',
                                                         'register_form': register_form})
            password = request.POST.get('password', '')
...
class ActiveUserView(View):
    def get(self, request, active_code):
        ...
        return render(request, 'logintest.html', {'msg': 'User activated'})
    ...
    def post(self, request):
        ...
        if forget_form.is_valid():
            ...
            if send_register_email(email, send_type='forget'):
                ...
                return render(request, 'forgetpass.html',{'mailmsg':mailmsg})
            else:
...
class ModifyPwdView(View):
    def post(self, request):
        ...
        if modify_pwd_form.is_valid():
            ...
            user.save()
            return render(request, 'logintest.html', {'msg': 'Password reset complete'})
        else:
            email = request.POST.get('email')
            return render(request, 'password_reset.html',
...
class UserInfoView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        return render(request, 'userdefault.html', {'my_courses': my_courses
...
class MyCourseView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        my_courses = UserCourse.objects.filter(user=request.user).order_by("-add_time")
        try:
            page = request.GET.get('page', 1)
        except PageNotAnInteger:
            page = 1
        # Take 3 out here, each page shows 3
        p = Paginator(my_courses, 3, request=request)
        page = p.page(page)

        return render(request, 'mycoursetest.html', {
...
class IndexView(View):
    def get(self, request):
        print ("Check 1") #cause server error to view 500 pages
        # Upper part carousel
    ...
    def post(self, request):
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                Subject = 'Result From Contact Form'
